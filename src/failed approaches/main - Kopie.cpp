...
double KEEP_DISTANCE=15; //~the distance around which the ego-car regulates its speed to match up with car in front
...
double WEIGHT_COLL=1000;   //Does the trajectory lead to close distances to other cars or even collisions?
...
Cars_around next_car_closeby(vector<double> ego_car_data, vector<vector<double>> sensor_fusion){
    ...
    for(int i=0;i<sensor_fusion.size();i++){
        ...
        bool is_parallel_to_me=((distance_to_car<10) & (distance_to_car>-10));
...
Trajectory create_splines_xt_yt(vector<double> car_data,vector<double> previous_x, vector<double> previous_y, vector<double> goal, vector<double> map_waypoints_s, vector<double> map_waypoints_x, vector<double> map_waypoints_y, vector<vector<double>> sensor_fusion){
    ...
    */
    ...
    double time_between_goal_velocity_and_goal=(distance_to_goal-distance_to_goal_vel)/goal_velocity;
    ...
    */
    ...
    double last_x=car_data[0];
    double last_y=car_data[1];
    double last_v_x=cos(car_data[4])*car_data[5];//sin, cos???
    double last_v_y=sin(car_data[4])*car_data[5];
    double last_acc_x =0;
    double last_acc_y =0;

    double time_step=OVERALL_DT*5;

    for(float time=t_0; time<((t_1-t_0)/3+t_0);time+=time_step){
        // speed above Vmax?
        double x=xt(time);
        double y=yt(time);
        ...
        //acceleration
        double accl_tot=sqrt(acc_x*acc_x+acc_y*acc_y); //omitted sqrt, unnecessary computation, can be taken into account in the following steps
        if(accl_tot<100) {accl_tot = 0;}
        cost_accl+=accl_tot;

        // Jerk?
        //double jerk_tot=sqrt(jerk_x*jerk_x+jerk_y*jerk_y); //omitted sqrt, unnecessary computation, can be taken into account in the following steps
        //if(jerk_tot<100) {jerk_tot = 0;}
        //cost_jerk+=jerk_tot;

        //Collision
        double closest_car=1-closest_car_at_t(time, x, y, sensor_fusion);
        //cout<<2-closest_car<<" --> ";
        double c3=closest_car*closest_car*closest_car;
        if(c3<0) {c3 = 0;} //only count positive values
        cost_collision+=exp(-time)*c3 ;//weight by distance along path
        last_x=x;
        last_y=y;
        last_v_x=v_x;
        last_v_y=v_y;
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            double near_goal = 50;
          	...
          	cout<<" is changing lanes "<<IS_CHANGING_LANES<<endl;

          	//STAY IN LANE
          	//goal --> goal s, goal d, goal v
          	//vector<double> goal;
...
//            bool crash_soon=0;
//            if (path_size>0){
//                crash_soon = closest_car_at_t((path_size-1)*OVERALL_DT,previous_path_x[path_size-1],previous_path_y[path_size-1],sensor_fusion)<2;
//            }
//
//            //emergency braking
//            if(crash_now | crash_soon | cars_closeby.distance<3 | cars_closeby.distance_l<3 | cars_closeby.distance_r<3 ){
//
//                double min_distance=path_size*OVERALL_DT*VMAX;
//                double min_distance_braking=0.5*VMAX*VMAX/A_MAX;
//                goals.push_back({car_s+min_distance+min_distance_braking+2, CURRENT_LANE*4+2, 1});
